...
function Navbar({ toggleSidebar }) {
    ...
    const handleViewDemande = async (demandeId) => {
        try {
            const response = await axios.get(`http://localhost:8080/api/demande/getDemandeById/${demandeId}`);
        ...
        <nav className="navbar navbar-expand bg-light navbar-light sticky-top px-4 py-0">
                    ...
                    {showMessages && messages.length > 0 && (
                        <div className="dropdown-menu dropdown-menu-end bg-light border-0 rounded-0 rounded-bottom m-0 show" style={{ width: '250px', maxHeight: '300px', overflowY: 'auto' }}>
                            {messages.slice().reverse().map((message, index) => (
                                <div key={index} className="dropdown-item" style={{ backgroundColor:'#3CB371'}}>
                                    ...
                                    <div className="text-center mt-2" style={{ whiteSpace: 'pre-wrap' }}>
                                        ...
                                        {message.expediteur.role === "Admin" && message.demande.id && (
                                        <button className="btn btn-link" onClick={() => handleViewDemande(message.demande.id)}>Voir détails</button>
                                    )}
                                    </div>
                                    <hr />
                                </div>
            ...
            <Modal show={showModal} onHide={handleCloseModal}>
                ...
                <Modal.Body>
                    {selectedDemande ? (
                        <div>
                            <p>Nom et prenom : {selectedDemande.nomPrenom}</p>
                            <p>INAM : {selectedDemande.inam}</p>
                            <p>Commentaire de la CNAM : {selectedDemande.commentaire}</p>
                        </div>
                    ) : (
                        <div>Loading...</div>
                    )}
